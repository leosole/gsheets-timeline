...
const BASE_LEGEND_ITEMS = [
  ...
  {
    label: "Concluído antes do previsto",
    kind: "swatch",
...
export const Legend: React.FC<LegendProps> = ({
  ...
  const handleColorChange = (label: string, nextColor: string) => {
    ...
    window.dispatchEvent(
      new CustomEvent("timeline:status-color-change", {
        detail: { label, color: safeColor },
      }),
    );
          ...
          <div key={item.label} className="flex items-center gap-2">
            ...
            {hasCustomStatusField && (
              <input
                ...
                className="hidden"
                onChange={(event) =>
                  handleColorChange(item.label, event.target.value)
                }
              />
            )}
