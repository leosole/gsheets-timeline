...
export interface BarMetrics {
  ...
  customColors?: { bg: string; darkBorder: string; lightBorder: string };
}
...
const pickPrimaryStatus = (statuses: string[]): string =>
  statuses.sort(
    (a, b) => (STATUS_SEVERITY[b] || 0) - (STATUS_SEVERITY[a] || 0),
  )[0] || "Não iniciado";
...
export const getStatusColorValue = (
  status: string,
  ...
  if (custom) {
    return {
      bg: custom,
      darkBorder: `color-mix(in oklab, ${custom} 80%, black 20%);`,
      lightBorder: `color-mix(in oklab, ${custom} 80%, white 20%);`,
    };
  }
...
export const calculateBarMetrics = (
  ...
  const colors = !hasExplicitStatus
    ...
    : statuses.includes("Atrasado") && !end
      ? getStatusColors("Fazendo")
      : getStatusColors(status);
  const customColors = hasExplicitStatus
    ? getStatusColorValue(status, statusColors)
    : undefined;
  ...
  const endPx = calculateDatePosition(
    barEnd.format("YYYY-MM-DD"),
    timelineData,
  );
  const endPxValue =
    endPx >= 0 ? endPx : timelineData.totalDays * timelineData.daySize;
  ...
  if (due) {
    plannedEndPx = calculateDatePosition(
    ...
    if (end && end.isBefore(due)) {
      ...
      const compPx = calculateDatePosition(
    ...
    if (isActuallyOverdue && plannedEndPx !== undefined) {
      ...
      if (plannedEndPx > 0) {
        ...
        overdueWidth = Math.max(
          (overdueEndPx >= 0
            ? overdueEndPx
            : now.diff(dayjs(timelineData.days[0].date), "day") *
                timelineData.daySize +
              timelineData.daySize / 2) - plannedEndPx,
          0,
        );
